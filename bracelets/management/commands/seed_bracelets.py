...
class Command(BaseCommand):
    help = 'Seeds the database with a default collection of bracelets.'

    def handle(self, *args, **options):
        if Bracelet.objects.exists():
            self.stdout.write(self.style.WARNING('Bracelet data already exists. No seeds were created.'))
            return

        self.stdout.write('Seeding default bracelets...')
        for item in BRACELET_SEED_DATA:
            Bracelet.objects.create(**item)

        self.stdout.write(self.style.SUCCESS('Successfully seeded the bracelet collection.'))
